...
def test_direct_answer_path_never_touches_tool_manager(
    generator, stub_tool_manager, openai_builders
):
    response = openai_builders.completion(
        "stop", openai_builders.message(content="Paris is the capital of France.")
    )
    ...
    result = generator.generate_response(
        ...
        tools=[
            {"name": "search_course_content", "description": "d", "input_schema": {}}
        ],
...
def test_no_tools_passed_means_no_tools_key_in_api_params(generator, openai_builders):
    response = openai_builders.completion(
        "stop", openai_builders.message(content="Some answer.")
    )
    generator.client.chat.completions.create.return_value = response
...
def test_tool_call_invokes_execute_tool_with_parsed_arguments(
    generator, stub_tool_manager, openai_builders
):
    tool_call = openai_builders.tool_call(
        "call_1",
        "search_course_content",
        json.dumps({"query": "X", "course_name": "Y"}),
    )
    ...
    second_response = openai_builders.completion(
        "stop", openai_builders.message(content="Final answer.")
    )
    ...
    stub_tool_manager.execute_tool.assert_called_once_with(
        "search_course_content", query="X", course_name="Y"
    )


def test_final_response_returned_after_tool_execution(
    generator, stub_tool_manager, openai_builders
):
    tool_call = openai_builders.tool_call(
        "call_1", "search_course_content", json.dumps({"query": "X"})
    )
    first_response = openai_builders.completion(
        "tool_calls", openai_builders.message(content=None, tool_calls=[tool_call])
    )
...
def test_second_call_assistant_message_echoes_tool_calls_exactly(
    generator, stub_tool_manager, openai_builders
):
    tool_call = openai_builders.tool_call(
        "call_1", "search_course_content", json.dumps({"query": "X"})
    ...
    second_call_messages = generator.client.chat.completions.create.call_args_list[
        1
    ].kwargs["messages"]
...
def test_second_call_tool_result_message_follows_assistant_message(
    generator, stub_tool_manager, openai_builders
):
    tool_call = openai_builders.tool_call(
        "call_1", "search_course_content", json.dumps({"query": "X"})
...
def test_round_two_call_retains_tools_and_tool_choice_keys(
    generator, stub_tool_manager, openai_builders
):
    ...
    second_call_kwargs = generator.client.chat.completions.create.call_args_list[
        1
    ].kwargs
...
def test_multiple_parallel_tool_calls_all_executed_in_order(
    generator, stub_tool_manager, openai_builders
):
    tool_call_1 = openai_builders.tool_call(
        "call_1", "search_course_content", json.dumps({"query": "A"})
    )
    tool_call_2 = openai_builders.tool_call(
        "call_2", "search_course_content", json.dumps({"query": "B"})
    )
    first_response = openai_builders.completion(
        "tool_calls",
        openai_builders.message(content=None, tool_calls=[tool_call_1, tool_call_2]),
    )
    second_response = openai_builders.completion(
        "stop", openai_builders.message(content="Final answer.")
    )
...
def test_none_content_serializes_as_none_not_string(
    generator, stub_tool_manager, openai_builders
):
    tool_call = openai_builders.tool_call(
        "call_1", "search_course_content", json.dumps({"query": "X"})
...
def test_tool_calls_finish_reason_but_no_tool_manager_falls_through_to_direct_content(
    generator, openai_builders
):
    tool_call = openai_builders.tool_call(
        "call_1", "search_course_content", json.dumps({"query": "X"})
    )
    response = openai_builders.completion(
        "tool_calls",
        openai_builders.message(content="fallback text", tool_calls=[tool_call]),
    )
    generator.client.chat.completions.create.return_value = response
...
def test_round_two_natural_early_termination_returns_content_directly(
    generator, stub_tool_manager, openai_builders
):
...
def test_sequential_two_round_tool_calls_execute_tool_called_with_correct_args_in_order(
    ...
    round_one_call = openai_builders.tool_call(
        "call_1",
        "search_course_content",
        json.dumps({"course_name": "Course X", "lesson_number": 4}),
    )
    round_two_call = openai_builders.tool_call(
        "call_2", "search_course_content", json.dumps({"query": "Prompt Compression"})
    )
    first_response = openai_builders.completion(
        "tool_calls", openai_builders.message(content=None, tool_calls=[round_one_call])
    )
    second_response = openai_builders.completion(
        "tool_calls", openai_builders.message(content=None, tool_calls=[round_two_call])
    )
    third_response = openai_builders.completion(
        "stop", openai_builders.message(content="Complete answer.")
    )
    generator.client.chat.completions.create.side_effect = [
        first_response,
        second_response,
    ...
    stub_tool_manager.execute_tool.side_effect = [
        "Lesson 4 is about Prompt Compression",
        "Found in Course Y",
    ]
    ...
    assert second_call_messages == [
        ...
        {
            "role": "user",
            "content": "Find a course covering the same topic as lesson 4 of Course X",
        },
        {
            "role": "assistant",
            "content": None,
            "tool_calls": [
                {
                    ...
                    "function": {
                        ...
                        "arguments": json.dumps(
                            {"course_name": "Course X", "lesson_number": 4}
                        ),
                    },
                }
            ],
        },
        {
            "role": "tool",
            "tool_call_id": "call_1",
            "content": "Lesson 4 is about Prompt Compression",
        },
    ]


def test_both_rounds_call_tools_triggers_final_synthesis_call(
    generator, stub_tool_manager, openai_builders
):
    ...
    third_response = openai_builders.completion(
        "stop", openai_builders.message(content="Synthesized answer.")
    )
    generator.client.chat.completions.create.side_effect = [
        first_response,
        second_response,
    ...
    third_call_kwargs = generator.client.chat.completions.create.call_args_list[
        2
    ].kwargs
    ...
    assert (
        len(third_call_messages) == 6
    )  # system, user, assistant1, tool1, assistant2, tool2
    assert [m["role"] for m in third_call_messages] == [
        "system",
        "user",
        "assistant",
        "tool",
...
def test_hard_tool_failure_terminates_early_into_final_synthesis_call(
    generator, stub_tool_manager, openai_builders
):
    ...
    second_response = openai_builders.completion(
        "stop", openai_builders.message(content="I couldn't complete that search.")
    )
    generator.client.chat.completions.create.side_effect = [
        first_response,
        second_response,
    ]
    stub_tool_manager.execute_tool.side_effect = RuntimeError(
...
def test_unknown_tool_name_result_does_not_terminate_loop(
    generator, stub_tool_manager, openai_builders
):
    ...
    tool_call = openai_builders.tool_call(
        "call_1", "nonexistent_tool", json.dumps({"query": "X"})
    )
    first_response = openai_builders.completion(
        "tool_calls", openai_builders.message(content=None, tool_calls=[tool_call])
    )
...
def test_malformed_tool_call_arguments_json_degrades_gracefully(
    generator, stub_tool_manager, openai_builders
):
    tool_call = openai_builders.tool_call(
...
def test_tool_call_with_unexpected_argument_key_degrades_gracefully_end_to_end(
    ...
    tool_call = openai_builders.tool_call(
        "call_1",
        "search_course_content",
        json.dumps({"query": "x", "bogus_field": "y"}),
    )
    first_response = openai_builders.completion(
        "tool_calls", openai_builders.message(content=None, tool_calls=[tool_call])
    )
...
def test_openai_api_error_propagates_uncaught(generator, stub_tool_manager):
    ...
    with pytest.raises(FakeAPIError):
        generator.generate_response(
            query="Any question", tool_manager=stub_tool_manager
        )
