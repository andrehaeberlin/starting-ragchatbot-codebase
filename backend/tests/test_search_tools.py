...
def test_execute_with_course_name_filter_real_store(course_search_tool_real):
    result = course_search_tool_real.execute(
        query="fixtures", course_name="Advanced Fixtures"
    )
...
def test_execute_with_lesson_number_filter_real_store(course_search_tool_real):
    result = course_search_tool_real.execute(
        query="assertions", course_name="Intro to Testing", lesson_number=2
    )
    assert "Lesson 2" in result


def test_execute_with_combined_course_and_lesson_filter_real_store(
    course_search_tool_real,
):
    result = course_search_tool_real.execute(
...
def test_execute_empty_results_no_filters_message(
    course_search_tool_fake, fake_vector_store
):
...
def test_execute_empty_results_with_course_name_message_text(
    course_search_tool_fake, fake_vector_store
):
    fake_vector_store.search_return = SearchResults(
        documents=[], metadata=[], distances=[]
    )
    result = course_search_tool_fake.execute(
        query="anything", course_name="Intro to Testing"
    )
    assert result == "No relevant content found in course 'Intro to Testing'."


def test_execute_empty_results_with_lesson_number_message_text(
    course_search_tool_fake, fake_vector_store
):
    fake_vector_store.search_return = SearchResults(
        documents=[], metadata=[], distances=[]
...
def test_execute_empty_results_with_both_filters_message_text(
    course_search_tool_fake, fake_vector_store
):
    fake_vector_store.search_return = SearchResults(
        documents=[], metadata=[], distances=[]
    ...
    result = course_search_tool_fake.execute(
        query="anything", course_name="Intro to Testing", lesson_number=3
    )
    assert (
        result == "No relevant content found in course 'Intro to Testing' in lesson 3."
    )
...
def test_execute_propagates_vector_store_error_string_verbatim(
    course_search_tool_fake, fake_vector_store
):
...
def test_execute_course_name_not_found_returns_no_match_error(
    course_search_tool_fake, fake_vector_store
):
    fake_vector_store.search_return = SearchResults.empty(
        "No course found matching 'Nonexistent Course'"
    )
    result = course_search_tool_fake.execute(
        query="anything", course_name="Nonexistent Course"
    )
...
def test_last_sources_lesson_case_shape(course_search_tool_fake, fake_vector_store):
    ...
    fake_vector_store.lesson_links[("Intro to Testing", 1)] = (
...
def test_last_sources_course_link_fallback_no_lesson_number(
    course_search_tool_fake, fake_vector_store
):
    fake_vector_store.search_return = SearchResults(
...
def test_last_sources_overwritten_not_accumulated_across_calls(
    ...
    assert (
        course_search_tool_fake.last_sources[0]["text"]
        == "Advanced Fixtures - Lesson 1"
    )


def test_last_sources_cleared_on_subsequent_error_or_empty_result(
    course_search_tool_fake, fake_vector_store
):
...
def test_tool_manager_execute_tool_dispatches_to_correct_tool(
    tool_manager_fake, fake_vector_store
):
    fake_vector_store.search_return = SearchResults(
        documents=[], metadata=[], distances=[]
    )
...
def test_tool_manager_execute_tool_unknown_name_returns_error_string_not_raise(
    tool_manager_fake,
):
...
def test_tool_manager_get_last_sources_scans_all_registered_tools(
    tool_manager_fake, fake_vector_store
):
    fake_vector_store.search_return = SearchResults(
        documents=["doc"],
